import { el } from "./redom.js"

export default [
    el("img.image", { src: "./art/AtNoon.jpg" }), //
    el("img.image", { src: "./art/water-bubbles.jpg" }),
    el("img.image", { src: "./art/Heaven.png" }),
    el("img.image", { src: "./art/CelestialMikaya.png" }),
    el("embed.pdf", { src: "./art/bruh.pdf" }),
    el("embed.pdf", { src: "./art/INeverLeaveYouBehind.pdf" }),
    el("audio.audio", { controls: "controls", src: "./art/ModernSoundscape.wav" }),
    el("audio.audio", { controls: "controls", src: "./art/Piano.mp3" }),
    el(
        "a",
        {
            href:
                "http://threejsplaygnd.brangerbriz.com/editor/#",
        },
        el("button.button button3", "Link"),
    ),
    el("a", { href: "https://skfb.ly/6HHyy" }, el("button.button button3", "Link")),
]
